...
async fn client_exists(pool: &SqlitePool, client_id: &str) -> bool {
    sqlx::query_scalar::<_, i64>(r#"SELECT COUNT(*) FROM oauth_clients WHERE "client_id" = $1"#)
        .bind(client_id)
        .fetch_one(pool)
        .await
        .unwrap_or(0)
...
async fn token_from_code(
    ...
    if now_secs() > expires_at {
        let _ = sqlx::query(r#"DELETE FROM oauth_codes WHERE "code" = $1"#)
            .bind(&code)
            .execute(pool)
            .await;
        return Err((
...
async fn token_from_refresh(
    ...
    let expires_at: u64 = expires_at.parse().unwrap_or(0);
    if now_secs() > expires_at {
        let _ = sqlx::query(r#"DELETE FROM refresh_tokens WHERE "token_hash" = $1"#)
            .bind(&hash)
            .execute(pool)
            .await;
        return Err((
...
pub async fn me(
    State(pool): State<Arc<SqlitePool>>,
    headers: HeaderMap,
) -> Result<Json<MeResponse>, (StatusCode, Json<AuthError>)> {
    let secret = jwt_secret().map_err(|_| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(AuthError {
...
mod tests {
    ...
    #[tokio::test]
    async fn pkce_login_issues_jwt_and_me() {
        unsafe {
        ...
        let me = me(State(Arc::new(pool.clone())), headers).await.unwrap();
        assert_eq!(me.username, "eyan");
        assert_eq!(me.user_type, "Human");
        assert_eq!(me.name, "Eyan");
    }
